...
test_in_ipi_template = """
<simulation verbosity="high">
  <output prefix="simulation">
      <properties stride='10' filename='out'>
        [ step, time{{picosecond}}, temperature{{kelvin}}, density{{g/cm3}} ]
      </properties>
      <trajectory filename='x_centroid' stride='{xv_stride}' cell_units='angstrom'> x_centroid{{angstrom}} </trajectory>
      <trajectory filename='v_centroid' stride='{xv_stride}' cell_units='angstrom'> v_centroid{{m/s}} </trajectory>
<checkpoint filename="chk" stride="{check_stride}" overwrite="true"/>
  </output>
  <prng><seed> {seed}</seed></prng>
  <total_steps>{md_steps}</total_steps>
