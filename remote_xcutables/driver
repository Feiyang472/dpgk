...
class BaseDriver(object):
    ...
    def posdata(self):
        ...
        energy, force, virial = self.grad(self.crd, self.cell)
        self.virial = virial # unit: H 
        self.energy = energy # unit: J
        self.force =  force # unit: J/m
        self.ifForce = True
    ...
    def getforce(self):
        ...
        for f in self.force.ravel():
            self.socket.send(struct.pack("d", f / (EH / BOHR)))  # Force unit: xx
        #virial = np.diag((self.force * self.crd).sum(axis=0)).ravel() / EH
        for v in self.virial.ravel():
